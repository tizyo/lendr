<template>
  <AppLayout>
    <template #header>
      <div>
        <h1 class="text-2xl font-bold text-neutral-900">Batch Payment Upload</h1>
        <p class="text-sm text-neutral-500 mt-0.5">Upload a CSV file to record multiple payments at once</p>
      </div>
    </template>
    ...
    <div class="max-w-2xl space-y-6">
      ...
      <div class="lendr-card p-6">
        ...
        <p class="text-sm text-neutral-500 mb-4">
          Required columns: <code class="bg-neutral-100 px-1 rounded text-xs">loan_number</code>, <code class="bg-neutral-100 px-1 rounded text-xs">amount</code>.
          Optional: <code class="bg-neutral-100 px-1 rounded text-xs">payment_date</code>, <code class="bg-neutral-100 px-1 rounded text-xs">payment_method</code>, <code class="bg-neutral-100 px-1 rounded text-xs">reference</code>, <code class="bg-neutral-100 px-1 rounded text-xs">notes</code>.
        </p>
      ...
      <!-- Upload -->
      <div class="lendr-card p-6">
        <h2 class="text-base font-semibold text-neutral-800 mb-3">2. Upload Payments File</h2>
        ...
        <div class="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
          <strong>Warning:</strong> Payment recording cannot be undone. Please verify your data before uploading.
        </div>
        ...
        <div class="mt-4 flex gap-3">
          ...
          >
            ...
            {{ uploading ? 'Processing…' : 'Upload & Record Payments' }}
          </button>
        </div>
      </div>
      ...
      <div v-if="result" class="lendr-card p-6 space-y-4">
        <h2 class="text-base font-semibold text-neutral-800">3. Results</h2>

        <div class="grid grid-cols-3 gap-4">
          <div class="text-center p-3 bg-emerald-50 rounded-xl">
            <p class="text-2xl font-bold text-emerald-700">{{ result.results.processed }}</p>
            <p class="text-xs text-neutral-500 mt-0.5">Processed</p>
          </div>
          ...
          <div class="text-center p-3 bg-neutral-50 rounded-xl">
            <p class="text-2xl font-bold text-neutral-700">{{ result.results.processed + result.results.skipped }}</p>
...
function downloadTemplate() {
  const sample = 'LN-202601-00001,500.00,2026-01-15,cash,REF123,January instalment'
...
async function upload() {
  ...
  try {
    const { data } = await axios.post(route('bulk.payments.upload'), fd, {
