...
class DisburseMobileMoneyJob implements ShouldQueue
{
    ...
    private function pawapay(Loan $loan, string $phone): void
    {
        ...
        if (! $apiKey) { $this->unsupported($loan, 'pawapay_api_key not configured'); return; }
    ...
    private function airtel(Loan $loan, string $phone): void
    {
        ...
        if (! $clientId || ! $clientSecret) { $this->unsupported($loan, 'Airtel credentials not configured'); return; }
    ...
    private function zamtel(Loan $loan, string $phone): void
    {
        ...
        if (! $apiKey || ! $baseUrl) { $this->unsupported($loan, 'Zamtel credentials not configured'); return; }
