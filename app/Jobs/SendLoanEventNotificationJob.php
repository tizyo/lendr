...
class SendLoanEventNotificationJob implements ShouldQueue
{
    ...
    private function buildMessage(Loan $loan): ?string
    {
        ...
        return match ($this->event) {
            'approved' =>
                "Hi {$name}, your LENDR loan {$number} has been approved! Our team will be in touch shortly. Thank you.",

            'disbursed' =>
                "Hi {$name}, your loan {$number} has been disbursed. Please check your account. Outstanding balance: ZMW {$amount}.",

            'payment_received' =>
                "Hi {$name}, we received your payment of ZMW {$paid} for loan {$number}. Remaining balance: ZMW {$amount}. Thank you!",

            'overdue' =>
                "Hi {$name}, your loan {$number} has an overdue installment. Outstanding: ZMW {$amount}. Please pay to avoid further penalties.",

            'defaulted' =>
                "Hi {$name}, your loan {$number} has been marked as defaulted due to non-payment. Please contact us immediately.",

            'upcoming_payment' =>
                "Reminder: Hi {$name}, your loan {$number} payment of ZMW {$this->context['amount_due']} is due on {$dueDate}. Please ensure funds are available.",
